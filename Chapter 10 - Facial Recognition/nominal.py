...
def associations(dataset, nominal_columns=None, mark_columns=False, theil_u=False, plot=True, save_plot=False,
    ...
    if plot:
        ...
        cn = kwargs.get('cmap',None)
        if cn is None:
            colors = sns.color_palette("Reds")
        else:
            colors = sns.color_palette(cn)
        sns.heatmap(corr, cmap=colors, annot=kwargs.get('annot',True), fmt=kwargs.get('fmt','.2f'))
        if save_plot:
            plt.savefig("feature_associations.png")
            plt.savefig("feature_associations.svg", format="svg")
        plt.show()
    if return_results:
        return corr
